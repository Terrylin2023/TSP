...
def upper_bound(node):
    # Assuming the start node is random
    start_node = node.path[0]    
...
def find_best_starting_point(matrix):
    ...
    for start in range(len(matrix)):
        nn_path = NearestNeighbor(matrix, start)
        nn_cost = cost_of(nn_path, matrix)
        if nn_cost < best_cost:
            best_cost = nn_cost
            best_starting_point = start
...
best_starting_point = find_best_starting_point(matrix)
print("Best Starting Point:", best_starting_point)
initial_node = Node([best_starting_point], 0, matrix)
